...
int main(int argc,char* argv[]) {
	...
	for(evNo=l;evNo<h;++evNo) {
		...
		std::cout<<ScintVect[0]->GetPlane(0)->GetFiredStripsVector().size()<<std::endl;
                std::cout<<ScintVect[1]->GetPlane(0)->GetFiredStripsVector().size()<<std::endl;
		if(setup->EventDetected()) {
			std::cout<<"NON SHOWER"<<std::endl;
			NonShowerEvents++;
			setup->SetEventDetected("GLASS",evNo);
			if(setup->EventDetected()) {
				std::cout<<"SINGLEPIXEL"<<std::endl;
				SinglePixelEvents++;
